...
flux_healthy_path = Path('healthy/sec_flux.csv')
d_type = {'flux': float,
          'reaction': 'category',
          'metabolite': 'category'}
...
flux_CRC_path = Path('StageI_II/sec_flux.csv')
flux_CRC_df = pd.read_csv(flux_CRC_path, sep = ",", dtype = d_type, usecols = use_cols)

merged_df = pd.merge(flux_healthy_df, flux_CRC_df, on = ["reaction", "metabolite"], how = 'inner')
...
print("Top 5 differences shifted towards CRC")
print(merged_df.head())
print("\nTop 5 differences shifted towards healthy")
